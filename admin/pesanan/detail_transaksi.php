...
<div class="form-group row">
              ...
              <div class="col-sm-10">
                <table class="table table-bordered table-hover">
                  ...
                  <tbody>
                  <?php
                  $query_keranjang = mysqli_query($koneksi, "SELECT tb_barang.nama_barang, tb_barang.harga_barang, tb_detail_pesanan.jumlah_pesanan, tb_detail_pesanan.jumlah_pesanan * tb_barang.harga_barang AS subtotal_harga FROM tb_detail_pesanan LEFT JOIN tb_barang ON tb_barang.id_barang = tb_detail_pesanan.id_barang WHERE id_pesanan = '$id_pesanan' ORDER BY id_detail DESC");
                  while ($keranjang = mysqli_fetch_array($query_keranjang)) { ?>
                    <tr>
                      ...
                      <td class="text-center">Rp<?php echo number_format($keranjang['subtotal_harga'], 0, '.', '.'); ?></td>
                    </tr>
                  <?php } ?>
                  <?php
                  $total_belanja = mysqli_query($koneksi, "SELECT SUM(tb_detail_pesanan.jumlah_pesanan * tb_barang.harga_barang) AS total FROM tb_detail_pesanan LEFT JOIN tb_barang ON tb_barang.id_barang = tb_detail_pesanan.id_barang WHERE tb_detail_pesanan.id_pesanan = '$id_pesanan'");
                  $total_harga   = mysqli_fetch_array($total_belanja);
                  $ongkos_kirim  = $total_harga['total'] > 100000 ? 0 : 50000;
                  $total_pembayaran = $total_harga['total'] + $ongkos_kirim;
                  ?>
                  <tr>
                    <td colspan="3" class="text-right"><b>Total Harga</b></td>
                    <td class="text-center">Rp<?php echo number_format($total_harga['total'], 0, '.', '.'); ?></td>
                  </tr>
                  <tr>
